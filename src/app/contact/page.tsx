...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Replace with real form submission when API route is ready
        ...
        <div className="container-custom">
          ...
          >
            ...
            <p className="text-zinc-400 text-lg max-w-2xl">
              Headquartered in Bangalore with regional presence in Dubai and London.
            </p>
          </motion.div>
            ...
            {offices.map((office, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <span className="text-xs text-zinc-500 block">{office.region}</span>
                    <h3 className="text-white font-semibold">{office.city}, {office.country}</h3>
                  </div>
                </div>
                <div className="space-y-2 text-sm text-zinc-500">
                  <p>{office.address}</p>
                  <a href={`tel:${office.phone.replace(/\s/g, '')}`} className="hover:text-zinc-300 transition-colors block">
                    {office.phone}
                  </a>
                  <a href={`mailto:${office.email}`} className="text-primary-orange hover:underline block">
                    {office.email}
                  </a>
                  <p className="text-zinc-600">{office.timezone}</p>
                </div>
              </motion.div>
            ))}
          </div>
